...
contract MintToPoolScript is Script {
    function run() external {
        ...
        for (uint i = 0; i < 10; i++) {
            Token fanToken = Token(tokenAddresses[i]);
            fanToken.mint(poolAddresses[i], amount);
            console.log(string.concat(" Minted to ", tokenNames[i], " pool: ", vm.toString(amount / 1e18), " tokens"));
        }
